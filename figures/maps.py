...
metadata = pd.read_excel(
    path_mc.joinpath(r"01_metadata/peilbuizen_metadata.xlsx"), index_col=0
)

bemonstering = pd.read_excel(
    path_mc.joinpath(r"05_waterkwaliteitmonsters/bemonstering_10-08-2022.xlsx"),
    header=0,
    skiprows=[1],
).set_index("Peilbuis")
...
handmetingen = pd.read_csv(
    path_mc.joinpath(r"02_handmetingen/handmetingen_23052023.csv"),
    skiprows=19,
    delimiter=";",
    encoding="ISO-8859-1",
    index_col=0,
)
handmetingen = handmetingen.rename(
    columns={"Electrical Conductivity[ÂµS/cm]": "electrical_conductivity [µS/cm]"}
)
...
x = (
    data.sort_values("electrical_conductivity [µS/cm]")[
...
for peilbuis_id in data.index.to_list():
    try:
        fresh_water_head = pd.read_csv(
            path_mc.joinpath(f"03_divers/zoetwaterstijghoogte/{peilbuis_id}.csv"),
            parse_dates=True,
            index_col=0,
        )
        data.loc[peilbuis_id, "mean_fresh_water_head (m NAP)"] = fresh_water_head[
            "fresh_water_head (m NAP)"
        ].mean()
...
geometry = [
    Point(trans.transform(x, y)) for x, y in zip(data["X-coord"], data["Y-coord"])
]
...
plot_gdf.dropna(subset=[variable]).plot(
    ax=ax, color="black", marker="o", markersize=markersize
)

for idx, row in plot_gdf.dropna(subset=[variable])[mask_aquifer_1].iterrows():
    ax.annotate(
        text=str(np.round(row[variable], 2)),
...
if variable == "mean_fresh_water_head (m NAP)":
    ax.annotate(
        text="Noordzeekanaal \n (ca. -0.40 [m NAP])",
        xy=trans.transform(102850, 497900),
        size=textsize,
        color="darkslategrey",
        ha="center",
    )
    ax.annotate(
        text="Noordzee \n (-1.35 - 1.80 [m NAP])",
        xy=trans.transform(101350, 498000),
        size=textsize,
        color="darkslategrey",
        ha="center",
    )
...
ax.annotate(
    text=" x.xx: freatisch pakket",
    xy=trans.transform(101020, 499070),
    size=textsize,
    color="black",
)
ax.annotate(
    text=" x.xx: 1e watervoerende pakket",
    xy=trans.transform(101020, 499010),
    size=textsize,
    color="maroon",
)
ax.annotate(
    text=" x.xx: 2e watervoerende pakket",
    xy=trans.transform(101020, 498950),
    size=textsize,
    color="navy",
)
