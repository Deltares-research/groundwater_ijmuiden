...
excel = pd.read_excel(r'N:/Projects/11207500/11207510/B. Measurements and calculations/05_waterkwaliteitmonsters/bemonstering_10-08-2022.xlsx', 
                      header = [0], index_col = 0, skiprows = [1])
relevant_ions = ['Chloride', 'Sulfate', 'HCO3-', 'Sodium', 'Potassium', 'Magnesium', 'Calcium']
...
for location in locations_np:
    ...
    ax = sns.heatmap(scaled, annot=selection, cbar=False, linewidths=.5, fmt='.1f', cmap = 'Reds')
    ...
    xticks = ['$\\mathbf{Chloride}$ \n [mg/l]', '$\\mathbf{Sulfaat}$ \n [mg/l]', '$\\mathbf{HCO_{3}^{-}}$ \n [mg/l]',
              '$\\mathbf{Natrium}$ \n [mg/l]','$\\mathbf{Kalium}$ \n [mg/l]', '$\\mathbf{Magnesium}$ \n [mg/l]',
              '$\\mathbf{Calcium}$ \n [mg/l]', 
              ]
    hline = []
    new_ylabels = []
    for locatie, pakket_list in ylabel_mapping.items():
        if ('watervoerend' in pakket_list[0]):
            pakket_list[0] = "{}  {}".format('$\mathbf{'+locatie+'}$', pakket_list[0])
    ...
    plt.tight_layout()
    fig.savefig(f'N:/Projects/11207500/11207510/B. Measurements and calculations/05_waterkwaliteitmonsters/webviewer/{location}.png')
    plt.close()


#%%
fig, ax = plt.subplots(figsize=(10,3))  
ax = sns.heatmap(table.loc['D'], annot=table.loc['D'], cbar=False, linewidths=.5, fmt='.1f', cmap = 'Reds')
